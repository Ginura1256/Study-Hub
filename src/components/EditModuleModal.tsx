...
import { X, Edit3, Network, Server, Code2, Cloud, Shield, Check, Save } from 'lucide-react';
import { useStudyHub } from '@/context/StudyHubContext';

export const EditModuleModal: React.FC = () => {
  const { editingModule, setEditingModule, updateModule } = useStudyHub();
  ...
  useEffect(() => {
    if (editingModule) {
      ...
      setInstructor(editingModule.instructor);
      setCredits(editingModule.credits);
      setDescription(editingModule.description);
      setColor(editingModule.color as any);
      setIconName(editingModule.iconName as any);
    }
  }, [editingModule]);
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-base font-bold text-slate-900 dark:text-slate-100">
                Edit Module Details ({editingModule.code})
              </h3>
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Update module code, title, instructor, and settings
              </p>
            </div>
          </div>
          <button
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2 text-sm bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 text-slate-900 dark:text-slate-100 font-mono font-bold"
              />
            </div>
